...
#'
#' Migrated from `taxinfo::theme_idest()`.
#'
...
theme_idest <- function(
  ...
  if (inherits(grid, "character") | grid == TRUE) {
    ...
    ret <- ret + ggplot2::theme(panel.grid.minor = ggplot2::element_blank())
  ...
  if (inherits(axis, "character") | axis == TRUE) {
    ret <- ret +
      ggplot2::theme(
        axis.line = ggplot2::element_line(color = axis_col, linewidth = 0.15)
      )
  ...
  if (strip_back_grey) {
    ret <- ret +
      ggplot2::theme(
        ...
        panel.border = ggplot2::element_rect(color = "grey90", fill = NA)
      )
  }
...
#'  "Hokusai2", "Hokusai3")
#'
#'  Migrated from `taxinfo::idest_pal`.
#' @format A named list where each element is a list of three elements:
#'   \describe{
#'     \item{colors}{A character vector of hex color codes.}
#'     \item{order}{A numeric vector describing the perceptual ordering of the colors.}
#'     \item{colorblind}{Logical. Whether the palette was designed to be colorblind-safe.}
#'   }
#' @return A named list of palettes (see Format).
#' @export
#' @author Adrien Taudière
idest_pal <- list(
...
idest_colors <- function(
  ...
  if (is.null(palette) | is.numeric(palette_name)) {
    cli::cli_abort("Palette {.val {palette_name}} does not exist.")
  }
  ...
  if (direction %notin% c(1, -1)) {
    cli::cli_abort(
      "Direction must be 1 (standard) or -1 (reversed)."
    )
  }
  ...
  if (type == "discrete" && n > length(palette[[1]])) {
    cli::cli_abort(
      "Number of requested colors greater than what the discrete palette can offer; use type = 'continuous' instead."
    )
  }
